...
export interface TeamAwardCardProps extends Omit<Team, Exclude<keyof Base, 'id'>> {
  className?: string;
...
@observer
export class TeamAwardCard extends ObservedComponent<TeamAwardCardProps, typeof i18n> {
  static contextType = I18nContext;

  memberStore = activityStore.teamOf(this.props.hackathon.name).memberOf(this.props.id);

  workStore = activityStore.teamOf(this.props.hackathon.name).workOf(this.props.id);

  assignmentStore = activityStore.teamOf(this.props.hackathon.name).assignmentOf(this.props.id);

  renderDetail() {
    const i18n = this.observedContext,
      { membersCount } = this.props;
    const { t } = i18n;
        ...
        <Accordion.Item eventKey="work">
          ...
          <Accordion.Body>
            <ScrollList
              ...
              renderList={allItems => <SimpleTeamWorkListLayout size="sm" defaultData={allItems} />}
            />
          </Accordion.Body>
        </Accordion.Item>
        <Accordion.Item eventKey="award">
          ...
          <Accordion.Body>
            <ScrollList
              ...
              renderList={allItems => <TeamAwardAssignmentLayout defaultData={allItems} />}
            />
          </Accordion.Body>
        </Accordion.Item>
      </Accordion>
  ...
  render() {
    ...
    return (
      <div className={classNames('border p-2', className)}>
        ...
        <Button className="my-2 ms-3" variant="success" onClick={() => onAssign(id!)}>
